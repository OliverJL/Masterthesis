...
\begin{itemize}
	\item \textit{Instruction delegation} Requests from consumers are directly translated to instructions using architecture dependent on-chip features as described in \ref{sec:lrng-general}.
	This approach bypasses the entire logic of the Linux PRNG.
	\item \textit{Driver delegation} Requests from consumers are
\end{itemize}

  
The process of generating random numbers is via the Linux PRNG relies on data provided by hardware events. 
