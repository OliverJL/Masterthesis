...
%\ref{sec:int-rnd}
 ...
 During the \textsf{late\_init} phase of the boot process, a global, kernel internal vector of 64 byte length is initialized to values taken from the non-blocking pool. This vector is kept constant over the entire runtime of the system and used for \gls{MD5} conditioning by the kernel consumer interfaces \textsf{get\_random\_int/\_long}. Thus, the level of entropy provided by this vector has a significant impact on quality of random numbers which will be delivered by to kernel consumers.
