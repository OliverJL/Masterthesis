...
The input pool is a central entropy pool dedicated to accumulate noise from the input sources described in \ref{sec:inp-src-lprng} of length 512 byte. If noise is delivered by interrupt events, it will be pre-processed by the fast pool. It is not accessible by a consumer directly, but dedicated to develop the output pools during initialization phase and further to feed those if entropy is requested. The input pool is as conditioning pool and hence applies a SHA1 transformation on the extracted data before completing delivery to the requesting output pool \cite{kernlrandmc}. It tracks its internal state by an entropy counter which is incremented when entropy is added i.e. decreased when is extracted. The input pool   
