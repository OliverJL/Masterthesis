...
The input pool is a central entropy pool dedicated to accumulate noise from the input sources described in \ref{sec:inp-src-lprng} of length 512 byte. If noise is delivered by interrupt events it will be pre-processed by the fast pool.


It is not accessible by a consumer directly, but dedicated to deliver entropy to output pools if requested.
